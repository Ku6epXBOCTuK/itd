...
- **Enum-подобные типы** — используй `const object` с `as const`, а не union types. Пример: `export const EnemyState = { MOVING: "moving", ATTACKING: "attacking" } as const`
...
- **UI -> Игра (Действия):** Используем Event Bus. Когда игрок кликает "Построить башню", Svelte не вызывает функцию `buildTower()`. Он делает: `GameEngine.emit('spawn-enemy', { type: 'archer', x: 10 })`. Система внутри модуля `towers` слушает это событие и вызывает `factories.ts`.
